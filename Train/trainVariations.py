...
num_epochs_ALL = [10, 20, 5, 15]
num_classes = 3
learning_rate_ALL = [0.001, 0.0001, 0.00001]
ImageSizes = [32, 96, 160, 224]

for num_epochs in num_epochs_ALL:
    for learning_rate in learning_rate_ALL:
        for ImageSize in ImageSizes:
            if os.path.isfile("FinalResNet_{}Epoch_{}lr_{}size.txt".format(num_epochs, learning_rate, ImageSize)):
                continue
            ...
            root_path = '../Dataset/'
            model_path = "FinalResNet_{}Epoch_{}lr_{}size.pt".format(num_epochs,learning_rate,ImageSize)
            ...
            with torch.no_grad():
                ...
                OutputFile = "FinalResNet_{}Epoch_{}lr_{}size.txt".format(num_epochs, learning_rate, ImageSize)
                f = open(OutputFile, "w")
                f.write("Training Accuracy for Every 10 batches:\n")
                f.write(str(acc_list)+"\n\n")
                f.write("Validation Accuracy for Every 10 batches:\n")
                f.write(str(acc_list_val)+"\n\n")
                f.write("Testing Accuracy on Unseen Images:\n")
                f.write(str(correct/total) + "\n\n")
                f.write("\n\n")
                f.write("Classification Report:\n")
                f.write(classification_report(AllLabels, AllPredictions))
                f.write("\n\n\n")
                f.write("Confusion Matrix:\n")
                f.write(str(confusion_matrix(AllLabels, AllPredictions)))
                f.close()