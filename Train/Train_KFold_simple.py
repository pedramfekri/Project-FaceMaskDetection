...
datafolder = datasets.ImageFolder(root=root_path + dir, transform=transform_dict['src'])

fold = 10

kf = KFold(n_splits=fold, shuffle=True)
kf.get_n_splits(datafolder)
...
k = 0
for train_index, test_index in kf.split(datafolder):
    # print("TRAIN:", train_index, "TEST:", test_index)

    tests = torch.utils.data.Subset(datafolder, test_index)
    dataLoader_test = torch.utils.data.DataLoader(tests, batch_size=32, shuffle=True, drop_last=False, num_workers=0, pin_memory=False)
    trains = torch.utils.data.Subset(datafolder, train_index)
    dataLoader_train = torch.utils.data.DataLoader(trains, batch_size=32, shuffle=True, drop_last=False, num_workers=0, pin_memory=False)
